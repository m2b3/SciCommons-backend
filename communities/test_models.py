...
from .models import Community, Membership, Invitation, JoinRequest, CommunityArticle, ArticleSubmissionAssessment
...
class MembershipModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.membership1 = Membership.objects.create(user=self.user1, community=self.community)
        # Add user2 as a member through the Membership model
        self.membership2 = Membership.objects.create(user=self.user2, community=self.community)
    ...
    def test_create_membership(self):
        ...
        self.assertTrue(Membership.objects.filter(user=user3, community=self.community).exists())
    ...
    def test_membership_relationship(self):
        ...
        self.assertTrue(Membership.objects.filter(user=self.user1, community=self.community).exists())
        self.assertTrue(Membership.objects.filter(user=self.user2, community=self.community).exists())

    def test_membership_deletion_on_user_deletion(self):
        user1_id = self.user1.id
        community_id = self.community.id
        self.user1.delete()
        self.assertFalse(Membership.objects.filter(user_id=user1_id, community_id=community_id).exists())
...
class InvitationModelTest(TestCase):
    ...
    def test_create_invitation_with_email(self):
        ...
        self.assertIsNotNone(invitation.invited_at)  # Check that the invited_at timestamp is set
        self.assertEqual(str(invitation), f"Invitation for invitee@example.com to {self.community.name}")  # Check string representation
    ...
    def test_create_invitation_with_username(self):
        ...
        self.assertEqual(invitation.status, Invitation.PENDING)
        self.assertIsNotNone(invitation.invited_at)  # Check that the invited_at timestamp is set
        self.assertEqual(str(invitation), f"Invitation for invitee_user to {self.community.name}")  # Check string representation
    ...
    def test_create_invitation_without_email_and_username(self):
        ...
        self.assertEqual(invitation.status, Invitation.PENDING)
        self.assertIsNotNone(invitation.invited_at)  # Check that the invited_at timestamp is set
        self.assertEqual(str(invitation), f"Invitation to {self.community.name}")  # Check string representation
...
class JoinRequestModelTest(TestCase):
    ...
    def test_create_join_request(self):
        ...
        self.assertEqual(str(join_request), f"{self.user1} requesting to join {self.community}")
...
class CommunityArticleTestCase(TestCase):
    ...
    def test_community_article_creation(self):
        ...
        community_article = CommunityArticle.objects.create(article=self.article, community=self.community)
