...
from .models import Article, ArticlePDF, AnonymousIdentity,Review,ReviewVersion,ReviewComment,Reaction,Discussion,DiscussionComment
...
class ArticleModelTest(TestCase):
    def setUp(self):
        ...
        self.article_data = {
            ...
            "submitter": self.user,
            "faqs": [{"question": fake.sentence(), "answer": fake.paragraph()} for _ in range(2)],
        }
...
class ArticlePDFModelTest(TestCase):
    ...
    def test_create_article_pdf(self):
        article_pdf = ArticlePDF.objects.create(article=self.article, pdf_file_url=self.pdf_file)
...
class AnonymousIdentityModelTest(TestCase):
    ...
    def test_get_or_create_fake_name_retrieves_existing_identity(self):
        existing_identity = AnonymousIdentity.objects.create(
            user=self.user,
            article=self.article,
            fake_name="ExistingFakeName"
        )
        fake_name = AnonymousIdentity.get_or_create_fake_name(self.user, self.article)
...
class ReviewModelTest(TestCase):
    ...
    def test_update_review_versioning(self):
        ...
        self.assertEqual(review_versions.first().content, "This is a great article. I learned a lot.")
...
class ReviewVersionModelTest(TestCase):
    ...
    def test_create_review_version(self):
        ...
        self.assertEqual(review_version.content, "This is a great article. I learned a lot.")
...
class ReactionModelTest(TestCase):
    ...
    def test_reaction_str(self):
        reaction = Reaction.objects.create(**self.reaction_data)
        expected_str = f"{self.user.username} - {reaction.get_vote_display()} on {reaction.content_object}"
...
class DiscussionCommentModelTest(TestCase):
    ...
    def test_create_discussion_comment(self):
        discussion_comment = DiscussionComment.objects.create(**self.discussion_comment_data)
    ...
    def test_create_nested_discussion_comment(self):
        parent_comment = DiscussionComment.objects.create(**self.discussion_comment_data)
